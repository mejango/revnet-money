...
- Bendystraw transport tests permit bounded retry only for transient failures and never retry an abort.
- Production-browser tests render the create surface at 320, 390, 768, and 1280 pixels, block and reject every attempted non-local HTTP/WebSocket request, disable service workers, and assert security headers, Farcaster-compatible embeddability, layout containment, valid project links, core controls, keyboard use, page-error absence, and zero serious/critical accessibility findings.
...
- Some queries are hand-written outside the generated document set. Move them into reviewed GraphQL documents before treating codegen as complete schema coverage.
